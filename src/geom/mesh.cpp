...
Mesh::Mesh(std::vector<Eigen::Vector3d> verts,
...
Mesh Mesh::computeCH() const {
  ...
  quickhull::ConvexHull<float> hull =
      qh.getConvexHull(point_cloud, true, false, success);
...
Mesh Mesh::computeVCH() const {
  ...
  for (int32_t i = 0; i < ch.vertices.size(); i++) {
    new_verts.emplace_back(ch.vertices[i].getX(), ch.vertices[i].getY(),
                           ch.vertices[i].getZ());
  }
  for (int32_t i = 0; i < ch.faces.size(); i++) {
    const auto *e = &ch.edges[ch.faces[i]];
    const auto *next_e = e->getNextEdgeOfFace();
    auto a = e->getSourceVertex(), b = e->getTargetVertex(),
...
double Mesh::volume() const {
  ...
  for (auto const &tri : this->m_triangles) {
    ...
    volume += signed_tri_volume(this->m_verts[i0], this->m_verts[i1],
                                this->m_verts[i2]);
  }
  return abs(volume);
}
...
vector<Plane> Mesh::get_cutting_planes(const Edge &concave_edge, int k) {
  ...
  auto n1 = edge_tri_norm(concave_edge, t1),
       n2 = edge_tri_norm(concave_edge, t2);
  // Get Quaternions for n1 -> n2
  Quaterniond qa = Quaterniond::Identity(),
              qb = Quaterniond::FromTwoVectors(n1, n2);
  // slerp it k times
  for (int i = 0; i < k; i++) {
    double t = double(i) / k;
...
std::vector<Mesh> Mesh::cut_plane(Plane &p) {
  ...
  err = mcDispatch(
      ...
      vertices, faces, faceSizes, numVertices, numFaces, cutMeshVertices,
      cutMeshFaces,
  ...
  err = mcGetConnectedComponents(context, MC_CONNECTED_COMPONENT_TYPE_FRAGMENT,
                                 (uint32_t)connComps.size(), connComps.data(),
                                 NULL);
  ...
  for (int i = 0; i < (int)connComps.size(); ++i) {
    ...
    err = mcGetConnectedComponentData(
        ...
        numBytes, faceIndices.data(), NULL);
    ASSERT_NO_ERROR(err);
...
Mesh::sample_point_set(int resolution) const {
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    size_t index =
        std::distance(m_cdf.begin(), it); // index of triangle sampled

    // if lower_bound returns end, set it to be last index
    if (index == m_triangles.size())
      index = m_triangles.size() - 1;
    ...
    point_samples[i] =
        random_barycentric_coord(m_verts[0], m_verts[1], m_verts[2]);
  }
...
vector<Edge> Mesh::shared_edges(const Vector3i &tri1, const Vector3i &tri2) {
  vector<Edge> shared;
  auto edges1 = this->get_triangle_edges(tri1),
       edges2 = this->get_triangle_edges(tri2);
  for (auto &&e1 : edges1) {
    for (auto &&e2 : edges2) {
      if (e1 == e2)
        shared.push_back(e1);
